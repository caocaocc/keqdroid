...
void main() {
  ...
  group('правила по процессам', () {
    ...
    for (final mode in [
      ...
      test('macOS ${mode.name} keeps core bypass before application paths', () {
        const appPath = '/Applications/KEQDIS.app/Contents/MacOS/KEQDIS';
        const selectedPath = '/Applications/Browser.app/Contents/MacOS/Browser';
        final config = MihomoConfigGen.build(
          link,
          settings,
          socksPort: 2080,
          ...
          routingMode: mode,
          managedProcessPaths: const [selectedPath],
          appProcessPath: appPath,
          windows: false,
        );
        final rules = _rules(config);
        final selectedTarget = mode == AppRoutingMode.onlySelected
            ? MihomoConfigGen.proxyName
            : 'DIRECT';
        final selectedRule = 'PROCESS-PATH,$selectedPath,$selectedTarget';
        expect(config['find-process-mode'], 'strict');
        expect(rules.first, 'PROCESS-PATH,$appPath,DIRECT');
        expect(rules, contains(selectedRule));
        for (final core in ['keqrnel', 'mihomo', 'xray', 'sing-box']) {
          final bypass = 'PROCESS-NAME,$core,DIRECT';
          expect(rules.where((rule) => rule == bypass), hasLength(1));
          expect(rules.indexOf(bypass), greaterThan(0));
          expect(rules.indexOf(bypass), lessThan(rules.indexOf(selectedRule)));
        }
        expect(
          rules.where((rule) => rule.startsWith('PROCESS-')),
          everyElement(isNot(contains('.exe'))),
        );
      });
    }

    for (final scenario in [
      (name: 'macOS Proxy', tun: null),
      (
        name: 'Android supplied TUN',
        tun: const MihomoTunOptions(
          stack: 'gvisor',
          fromFileDescriptor: true,
          autoRoute: false,
      ...
      test('${scenario.name} excludes core and application process rules', () {
        final config = MihomoConfigGen.build(
          link,
          settings,
          socksPort: 2080,
          ...
          managedProcessNames: const ['Browser'],
          managedProcessPaths: const [
            '/Applications/Browser.app/Contents/MacOS/Browser',
          ],
          appProcessName: 'KEQDIS',
          appProcessPath: '/Applications/KEQDIS.app/Contents/MacOS/KEQDIS',
          windows: false,
        );
        expect(config['find-process-mode'], 'off');
        expect(
          _rules(config).where((rule) => rule.startsWith('PROCESS-')),
          isEmpty,
        );
        expect(_rules(config).last, 'MATCH,${MihomoConfigGen.proxyName}');
      });
    }
