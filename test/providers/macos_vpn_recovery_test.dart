...
void main() {
  ...
  test(
    'current-node handoff preserves macOS waiting recovery after inputs change',
    () async {
      ...
      await container
          ...
          .save(settings.copyWith(httpPort: settings.httpPort + 1));
      await Future<void>.delayed(Duration.zero);
      final generation = vpn.connectionGeneration;
      final before = starts;
      await vpn.selectServerManually(server, reconnectIfActive: true);
      expect(vpn.connectionGeneration, generation);
      expect(starts, before);
      expect(desktopRecoveryStatus.value, isNotNull);
      online = true;
      for (var i = 0; i < 4; i++) {
        await poll();
      }
