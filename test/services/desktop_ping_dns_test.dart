...
void main() {
  ...
  for (final protocol in ['vless', 'trojan']) {
    test(
      'desktop $protocol REALITY ping keeps upstream TLS and physical DNS',
      () {
        final config =
            jsonDecode(
                  MihomoConfigGen.generatePingConfig(
                    '$protocol://test@node.example:443?type=tcp&security=reality'
        ...
        expect(config['port'], 28150);
        expect(config['dns']['nameserver'], ['system']);
