...
void main() {
  ...
  test(
    'watchdog counters ignore hidden-window polling without changing UI rates',
    () async {
      await sample(100, 200);
      backend.setTrafficStatsPollingEnabled(false);
      backend.response = () async => (down: 500, up: 700);
      expect(await backend.sessionTrafficCounters(), (down: 500, up: 700));
      expect(backend.currentState.totalDownload, 100);
      expect(backend.currentState.totalUpload, 200);
      expectNoRate();
    },
  );

  test('watchdog discards samples from a replaced helper epoch', () async {
    final pending = Completer<_Counters?>();
    backend.response = () => pending.future;
    final sampling = backend.sessionTrafficCounters();
    snapshot = {...connected('first-session'), 'helperEpoch': 'new-helper'};
    await backend.getCurrentState();
    pending.complete((down: 999999, up: 999999));
    expect(await sampling, isNull);
    backend.response = () async => (down: 10, up: 20);
    expect(await backend.sessionTrafficCounters(), (down: 10, up: 20));
  });

  test('watchdog discards samples completing after stop', () async {
    final pending = Completer<_Counters?>();
    backend.response = () => pending.future;
    final sampling = backend.sessionTrafficCounters();
  ...
  test(
    'native cumulative failures count only new same-session increments',
    () async {
      final before = CoreDialFailures.count;
      snapshot = {...connected('first-session'), 'dialFailureCount': 5};
      await backend.getCurrentState(); // first report is a baseline
      await backend.getCurrentState();
      expect(CoreDialFailures.count, before);
      snapshot['dialFailureCount'] = 7;
      await backend.getCurrentState();
      await backend.getCurrentState();
      expect(CoreDialFailures.count, before + 2);
      snapshot['dialFailureCount'] = 1; // counter reset must not add negatives
      await backend.getCurrentState();
      snapshot['dialFailureCount'] = 2;
      await backend.getCurrentState();
      expect(CoreDialFailures.count, before + 3);
      snapshot = {
        ...connected('first-session'),
        'helperEpoch': 'new',
        'dialFailureCount': 100,
      };
      await backend.getCurrentState();
      expect(CoreDialFailures.count, before + 3);
      snapshot = connected('first-session'); // old helper has no field
      await backend.getCurrentState();
      expect(CoreDialFailures.count, before + 3);
    },
  );
