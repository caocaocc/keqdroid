...
void main() {
  ...
  for (final core in VpnBackend.values) {
    test(
      'Darwin ${core.name} keeps the supported Mips stack and DNS policy',
      () {
        ...
        final singbox = SingBoxTunConfigGen.generate(
          localSocksPort: 2080,
          socksUsername: 'user',
          socksPassword: 'password',
          serverIpToExclude: '203.0.113.8',
        ...
        final mihomo = MihomoConfigGen.build(
          'vless://test@203.0.113.8:443?security=none&type=tcp',
          settings,
          socksPort: 2080,
          httpPort: 2081,
          windows: false,
        ...
        final payload = MacOSSessionConfig.build(
          request: TunnelSessionRequest(
            ...
            mihomoConfig: jsonEncode(mihomo),
          ),
          sessionId: 'mips-${core.name}',
          context: context,
          apiPort: 23000,
          apiSecret: 'session-secret-123456',
        );
        final coreName = core == VpnBackend.mihomo ? 'mihomo' : 'keqrnel';
        final config =
            jsonDecode((payload['configurations'] as Map)[coreName] as String)
                as Map;
        final Map inbound;
        if (core == VpnBackend.mihomo) {
          inbound = config['tun'] as Map;
          expect(inbound['stack'], TunSettings.stackMips);
          expect(inbound.containsKey('device'), isFalse);
          expect(inbound.containsKey('strict-route'), isFalse);
          expect(config['interface-name'], 'en0');
          expect(config['dns'], mihomo['dns']);
        } else {
          inbound = (config['inbounds'] as List).first as Map;
          expect(inbound['stack'], TunSettings.defaultStack);
          expect(inbound['endpoint_independent_nat'], isTrue);
          expect(inbound.containsKey('interface_name'), isFalse);
          expect(inbound.containsKey('strict_route'), isFalse);
          expect(config['route']['default_interface'], 'en0');
          expect(config['dns'], jsonDecode(singbox)['dns']);
        }
        expect(settings.tun.stack, TunSettings.stackMips);
      },
    );
  }
