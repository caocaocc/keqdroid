...
class _DesktopHomeScreenState extends ConsumerState<DesktopHomeScreen>
    ...
    try {
      ...
      switch (action) {
        ...
        case 'selectServer':
          ...
          final changed = target.id != servers.activeServerId;
          final connected = ref.read(vpnStateProvider).value?.status == VpnStatus.connected;
          await vpn.selectServerManually(target);
          if (connected && changed) await vpn.reconnectToActiveServer();
      }
    } catch (error) {
      if (mounted) {
        await MacOSDesktopService.showWindow();
  ...
  Future<void> _hotkeyBestPingServer() async {
    ...
    await ref.read(vpnStateProvider.notifier).selectServerManually(best);
    final status = ref.read(vpnStateProvider).value?.status;
    if (status == VpnStatus.connected || status == VpnStatus.connecting) {
