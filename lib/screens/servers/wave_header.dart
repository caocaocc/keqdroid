...
/// Индикатор состояния туннеля: волнистый прогресс M3 Expressive целиком —
/// трек, активная часть и точка-стоп в конце.
///
/// Раньше здесь был один штрих без трека и точки, и он читался как нарисованная
/// синусоида, а не как индикатор. Числа взяты из спеки, а не выведены из толщины
/// штриха; амплитуда — потолок, выше начинается график синуса.
///
/// Поверх спеки индикатор несёт три канала: цвет — задержка сервера, амплитуда —
/// живой трафик (гаснет вместе с чипами трафика), форма — состояние подключения.
/// У состоявшегося соединения активная часть до конца трека не доходит:
/// соединение это состояние, а не задача с финишем, и полоса до края врала бы
/// про завершённую загрузку. Бегущий сегмент при подключении проходит трек
/// целиком.
