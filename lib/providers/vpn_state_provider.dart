...
class VpnStateNotifier extends AsyncNotifier<VpnState> {
  ...
  /// A deliberate desktop menu/hotkey selection hands control back to the user.
  Future<void> selectServerManually(ServerItem server) async {
    _invalidateAutoSelect();
    await ref.read(subscriptionsProvider.notifier).handAutoSelectTo(null);
    if (ref.mounted) await ref.read(serversProvider.notifier).setActive(server);
  }
