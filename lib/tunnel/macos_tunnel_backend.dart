...
class MacOSTunnelBackend extends TunnelBackend with DesktopTrafficStats {
  ...
  int? _seenDialFailures;
  String? _dialFailureSession;
  ...
  String? get activeSessionIdentity => _sessionId == null
      ? null
      : '${_snapshot['helperEpoch'] ?? _serviceInfo['helperEpoch']}/$_sessionId';
  ...
  void _clearSession() {
    _seenDialFailures = null;
    _dialFailureSession = null;
  ...
  void _acceptSnapshot(Map<String, dynamic> snapshot) {
    ...
    final failures = snapshot['dialFailureCount'];
    if (status == 'connected' &&
        incomingId != null &&
        failures is int &&
        failures >= 0) {
      final identity =
          '${snapshot['helperEpoch'] ?? _serviceInfo['helperEpoch']}/$incomingId';
      final previous = _dialFailureSession == identity
          ? _seenDialFailures
          : null;
      _dialFailureSession = identity;
      _seenDialFailures = failures;
      if (previous != null && failures >= previous) {
        CoreDialFailures.add(failures - previous);
      }
    }
  ...
  Future<({int down, int up})?> sessionTrafficCounters() async {
    ...
    final port = _apiPort;
    final generation = _operationGeneration;
    final identity = activeSessionIdentity;
    if (_disposed ||
        _stopRequested ||
        id == null ||
        port == null ||
