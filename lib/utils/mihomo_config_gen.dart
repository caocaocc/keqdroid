...
class MihomoConfigGen {
  ...
  ///
  /// Схемы у ядер разные, а поле в настройках одно, поэтому переводим. Суффикс
  /// `+local` («мимо роутинга» у xray) просто снимаем — у mihomo это и так
  /// поведение по умолчанию. `localhost` становится `system`. `h2c://` и
  /// `fakedns` mihomo не знает, и их выбрасываем: неизвестная схема роняет
  /// разбор конфига целиком, а с ним и подключение.
  ///
