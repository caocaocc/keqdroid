...
///
/// Считает разницей двух снимков: своих скоростей ядро не отдаёт, только
/// сколько всего набежало по каждому соединению.
///
/// Объём канала — сумма этих разниц за сессию, и он чуть занижен: закрывшееся
/// между опросами соединение недодаёт последнее, а совсем короткое может целиком
/// уместиться между снимками. Точный общий объём ядро отдаёт, но одной цифрой на
/// всё, и разложить её по каналам нечем — ставить в полосу три числа, которые не
/// сходятся, хуже.
///
/// Отсюда требование к вызывающему: опрашивать непрерывно, пока держится сессия.
/// Каждая пауза — дыра, и закрыть её потом можно лишь наполовину: у переживших
/// паузу соединений счётчики так и лежат в ядре накопленными (на этом стоит
/// [restore]), а от закрывшихся не осталось ничего.
