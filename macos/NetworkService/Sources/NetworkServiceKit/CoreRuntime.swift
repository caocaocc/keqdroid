...
public final class CoreProcess {
    ...
    private var output: CoreSessionLog
    public var log: Data { output.data }
    public var dialFailures: Int { output.dialFailures }

    init(name: String, executable: URL, arguments: [String], environment: [String: String], directory: URL, uid: uid_t, gid: gid_t, xrayLogThreshold: Int = 0) throws {
        self.name = name; self.executable = executable.path
        self.output = CoreSessionLog(xrayThreshold: xrayLogThreshold)
    ...
    public func collectOutput(maximumReads: Int = .max) {
        CoreOutputReader.drain(outputFD, maximumReads: maximumReads, onEOF: { self.output.finish() }) { data in
            output.append(data)
        }
    }
...
public final class CoreRuntime {
    ...
    public func spawn(name: String, configuration: String, directory: URL, request: SessionRequest, identity: ClientIdentity, context: NetworkContext) throws -> CoreProcess {
        ...
        return try CoreProcess(name: name, executable: executable, arguments: arguments, environment: environment, directory: coreDirectory, uid: uid, gid: gid, xrayLogThreshold: request.xrayLogThreshold)
    }
}
