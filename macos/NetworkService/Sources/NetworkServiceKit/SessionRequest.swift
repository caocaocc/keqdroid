...
public struct SessionRequest {
    ...
    public let xrayLogThreshold: Int
    ...
    public init(arguments: [String: Any]) throws {
        ...
        if let raw = arguments["xrayLogThreshold"] {
            guard let number = raw as? NSNumber, CFGetTypeID(number) != CFBooleanGetTypeID(),
                  number.doubleValue == Double(number.intValue), (0...4).contains(number.intValue),
                  core == "keqrnel" else { throw ServiceFailure("invalidRequest", "Invalid Xray display log level.") }
            xrayLogThreshold = number.intValue
        } else { xrayLogThreshold = 0 }
...
public enum ConfigPolicy {
    ...
    private enum DNSDataKind { case hosts, predefined, resolvers }
    private enum EmbeddedXrayScope { case outbound, xray, dns }

    private static func dnsName(_ value: String) -> Bool {
        value.utf8.count <= 253 && value.range(of: "^[A-Za-z0-9_-]+(\\.[A-Za-z0-9_-]+)*\\.?$", options: .regularExpression) != nil
    }

    private static func dnsResolver(_ value: String) -> Bool {
        guard value.utf8.count <= 4096, let decoded = value.removingPercentEncoding,
              !decoded.unicodeScalars.contains(where: { CharacterSet.controlCharacters.contains($0) }),
              !decoded.contains("\\") else { return false }
        if value == "system" || value == "system://" || isIPAddress(value) { return true }
        let qualified = value.contains("://") ? value : "udp://" + value
        guard let url = URLComponents(string: qualified), let scheme = url.scheme?.lowercased(),
              ["udp", "tcp", "tls", "http", "https", "quic", "dhcp", "rcode", "ts", "tailscale", "et", "easytier"].contains(scheme),
        ...
        guard isIPAddress(host) || dnsName(host), url.port.map({ (1...65535).contains($0) }) ?? true else { return false }
        if scheme == "rcode" {
            return ["success", "format_error", "server_failure", "name_error", "not_implemented", "refused"].contains(host)
        }
        return ["http", "https"].contains(scheme) || url.path.isEmpty
    }

    // Domain keys in these exact core-owned maps are data, not option names.
    // Keep values flat and typed so nested options cannot gain an exemption.
    private static func validateDNSData(_ value: Any, kind: DNSDataKind) throws {
        guard let entries = value as? [String: Any] else {
            throw ServiceFailure("unsafeConfiguration", "DNS data must be a flat domain map.")
        }
        for (name, raw) in entries {
            guard !name.isEmpty, name.utf8.count <= 1024, !name.contains("/"), !name.contains("\\"),
                  !name.unicodeScalars.contains(where: { CharacterSet.controlCharacters.contains($0) }) else {
                throw ServiceFailure("unsafeConfiguration", "Invalid DNS data key.")
            }
            let values: [String]
            if let text = raw as? String, kind != .predefined { values = [text] }
            else if let list = raw as? [String], !list.isEmpty { values = list }
            else { throw ServiceFailure("unsafeConfiguration", "DNS data values must be strings or nonempty string lists.") }
            let valid: Bool
            switch kind {
            case .predefined: valid = values.allSatisfy(isIPAddress)
            case .hosts: valid = values.allSatisfy(isIPAddress) || values.count == 1 && dnsName(values[0])
            case .resolvers: valid = values.allSatisfy(dnsResolver)
            }
            guard valid else { throw ServiceFailure("unsafeConfiguration", "DNS data contains an invalid address, alias or resolver.") }
        }
    }
    ...
    public static func validateJSON(_ root: [String: Any], core: String, mode: String, ports: Set<Int>, apiPort: Int, secret: String, lan: LANConfiguration? = nil) throws {
        ...
        func walk(_ node: Any, parents: [String], depth: Int, isTopLevelDNSServer: Bool = false,
                  isTopLevelMihomoProxy: Bool = false, mihomoPaths: MihomoHTTPPathScope? = nil,
                  dnsData: DNSDataKind? = nil, xrayScope: EmbeddedXrayScope? = nil) throws {
            count += 1
            guard depth < 48, count < 100_000 else { throw ServiceFailure("invalidConfiguration", "Configuration nesting or size exceeds limits.") }
            if let kind = dnsData {
                try validateDNSData(node, kind: kind)
                // Preserve global size/depth and file-URI checks for each value.
                for value in (node as! [String: Any]).values {
                    try walk(value, parents: parents, depth: depth + 1)
                }
                return
            }
            if let dictionary = node as? [String: Any] {
                ...
                for (key, value) in dictionary {
                    ...
                    let childDNSData: DNSDataKind?
                    if core == "mihomo", parents.isEmpty, depth == 0, key == "hosts" {
                        childDNSData = .hosts
                    } else if core == "mihomo", parents == ["dns"], depth == 1, key == "nameserver-policy" {
                        childDNSData = .resolvers
                    } else if xrayScope == .dns, key == "hosts" {
                        childDNSData = .hosts
                    } else if isTopLevelDNSServer, dictionary["type"] as? String == "hosts", key == "predefined" {
                        childDNSData = .predefined
                    } else { childDNSData = nil }
                    let childXrayScope: EmbeddedXrayScope?
                    if xrayScope == .outbound, dictionary["type"] as? String == "xray", key == "xray", value is [String: Any] {
                        childXrayScope = .xray
                    } else if xrayScope == .xray, key == "dns", value is [String: Any] {
                        childXrayScope = .dns
                    } else { childXrayScope = nil }
                    try walk(value, parents: parents + [key], depth: depth + 1, mihomoPaths: childPaths, dnsData: childDNSData, xrayScope: childXrayScope)
                }
                ...
                let embeddedOutbounds = core == "keqrnel" && parents == ["outbounds"] && depth == 1
                for item in array {
                    try walk(item, parents: parents, depth: depth + 1, isTopLevelDNSServer: dnsServers, isTopLevelMihomoProxy: mihomoProxies, xrayScope: embeddedOutbounds ? .outbound : nil)
                }
