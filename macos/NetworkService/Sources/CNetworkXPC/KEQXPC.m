...
int keq_process_lan_listener(pid_t pid, uint16_t port) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (tcp->tcpsi_state != TSI_S_LISTEN || ntohs((uint16_t)tcp->tcpsi_ini.insi_lport) != port) continue;
        const struct in_sockinfo *ip = &tcp->tcpsi_ini;
        // Go may represent 0.0.0.0 as a dual-stack IPv6 wildcard. The kernel's
        // IPv4 capability bit excludes IPv6-only sockets at the same address.
        if ((info.psi.soi_family == AF_INET && ip->insi_laddr.ina_46.i46a_addr4.s_addr == htonl(INADDR_ANY)) ||
            (info.psi.soi_family == AF_INET6 && (ip->insi_vflag & INI_IPV4) && IN6_IS_ADDR_UNSPECIFIED(&ip->insi_laddr.ina_6))) {
            result = 1; break;
        }
    }
    free(fds);
