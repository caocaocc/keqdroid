...
if CommandLine.arguments.count == 2 && CommandLine.arguments[1] == "--auto-select-logs-only" {
    print("Passed \(runAutoSelectLogChecks()) automatic selection log checks (no network operations).")
    exit(0)
}
...
if CommandLine.arguments.count == 2 && CommandLine.arguments[1] == "--dns-preferences-only" {
    print("Passed \(runDNSPreferencesChecks()) DNS preference validation checks (no network operations).")
    exit(0)
}
...
var checks = 0
checks += runAutoSelectLogChecks()
checks += runDNSPreferencesChecks()
...
expect(pipe(&logPipe) == 0 && fcntl(logPipe[0], F_SETFL, O_NONBLOCK) == 0 && fcntl(logPipe[1], F_SETFL, O_NONBLOCK) == 0, "create nonblocking synthetic core output pipe")
// Darwin may shrink a pipe to PIPE_BUF under memory pressure. A blocking
// 8192-byte first write can hang before the reader even starts.
let logChunkSize = Int(fpathconf(logPipe[1], _PC_PIPE_BUF))
expect(logChunkSize > 0 && logChunkSize <= 8192, "synthetic producer uses an atomic pipe-sized chunk")
let logPayload = Data((0..<(12 * logChunkSize)).map { UInt8($0 % 251) })
var writtenLogBytes = 0
var receivedLog = Data()
func produceLogChunk() {
    guard writtenLogBytes < logPayload.count else { return }
    let count = logPayload.withUnsafeBytes { Darwin.write(logPipe[1], $0.baseAddress!.advanced(by: writtenLogBytes), logChunkSize) }
    guard count == logChunkSize else { fputs("FAIL: synthetic pipe producer\n", stderr); exit(1) }
...
expect(firstLogRead == 8 * logChunkSize && receivedLog.count == firstLogRead, "startup log collection yields after eight reads even with a continuous producer")
let remainingLogRead = CoreOutputReader.drain(logPipe[0], maximumReads: 8, consume: consumeLog)
expect(remainingLogRead == 4 * logChunkSize && receivedLog == logPayload, "next log collection preserves all unread pipe bytes without loss")
