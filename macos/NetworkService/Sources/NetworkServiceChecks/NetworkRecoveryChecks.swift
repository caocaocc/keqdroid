...
func runNetworkRecoveryChecks() -> Int {
    ...
    check(keq_process_lan_listener(getppid(), wildcard.1) == 0, "live process without this listener is not LAN ready")
    close(wildcard.0)
    check(keq_process_lan_listener(getpid(), wildcard.1) == 0, "closed listener is not ready")

    func ipv6Listener(only: Int32, loopback: Bool = false) -> (Int32, UInt16) {
        let fd = socket(AF_INET6, SOCK_STREAM, 0)
        var only = only
        var address = sockaddr_in6(); address.sin6_len = UInt8(MemoryLayout<sockaddr_in6>.size)
        address.sin6_family = sa_family_t(AF_INET6)
        address.sin6_addr = loopback ? in6addr_loopback : in6addr_any
        let option = setsockopt(fd, IPPROTO_IPV6, IPV6_V6ONLY, &only, socklen_t(MemoryLayout<Int32>.size))
        let result = withUnsafePointer(to: &address) {
            $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { Darwin.bind(fd, $0, socklen_t(MemoryLayout<sockaddr_in6>.size)) }
        }
        guard fd >= 0, option == 0, result == 0, listen(fd, 8) == 0 else { fputs("FAIL: temporary IPv6 listener fixture\n", stderr); exit(1) }
        var size = socklen_t(MemoryLayout<sockaddr_in6>.size)
        _ = withUnsafeMutablePointer(to: &address) { $0.withMemoryRebound(to: sockaddr.self, capacity: 1) { getsockname(fd, $0, &size) } }
        return (fd, UInt16(bigEndian: address.sin6_port))
    }
    let dualStack = ipv6Listener(only: 0)
    check(keq_socket_ready("127.0.0.1", dualStack.1, 200) == 1, "dual-stack fixture accepts IPv4")
    check(keq_process_lan_listener(getpid(), dualStack.1) == 1, "owned dual-stack wildcard is LAN ready")
    check(keq_process_lan_listener(getppid(), dualStack.1) == 0, "dual-stack listener still requires exact owner")
    check(keq_lan_port_available(dualStack.1) == 0, "dual-stack wildcard occupies IPv4 LAN port")
    close(dualStack.0)
    check(keq_process_lan_listener(getpid(), dualStack.1) == 0, "closed dual-stack listener is not ready")
    let ipv6Only = ipv6Listener(only: 1)
    check(keq_socket_ready("127.0.0.1", ipv6Only.1, 200) == 0, "IPv6-only fixture rejects IPv4")
    check(keq_process_lan_listener(getpid(), ipv6Only.1) == 0, "IPv6-only wildcard cannot advertise IPv4 LAN")
    close(ipv6Only.0)
    let ipv6Loopback = ipv6Listener(only: 0, loopback: true)
    check(keq_process_lan_listener(getpid(), ipv6Loopback.1) == 0, "IPv6 loopback is not LAN ready")
    close(ipv6Loopback.0)
    return count
}
