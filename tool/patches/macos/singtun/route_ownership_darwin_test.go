...
func TestKeqdisRouteCloseIsIdempotentAndPreventsRestart(t *testing.T) {
	...
	poller, err := rawfile.NewPoller(stop.ReadFD, int(reader.Fd()))
	if err != nil {
		stop.Close()
		t.Fatal(err)
	}
	tun := &NativeTun{tunFile: reader, stopFd: stop, readPoller: poller, options: Options{EXP_ExternalConfiguration: true}}
	...
	if tun.stopFd.ReadFD != -1 || tun.stopFd.WriteFD != -1 {
		t.Fatal("stop descriptors survived close")
	}
	if _, err := poller.Wait(); err != unix.EBADF {
		t.Fatalf("poller survived close: %v", err)
	}
