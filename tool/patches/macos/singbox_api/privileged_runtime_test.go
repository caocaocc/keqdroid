...
func keqdisTestRouter(t *testing.T) http.Handler {
	...
	factory, err := log.New(log.Options{Context: ctx, Observable: true, DefaultWriter: io.Discard})
	if err != nil {
		t.Fatal(err)
	}
	ctx = service.ContextWithPtr(ctx, trafficcontrol.NewManager(nil))
	ctx = service.ContextWithPtr(ctx, urltest.NewHistoryStorage())
	ctx = service.ContextWithPtr(ctx, clashmode.NewManager(ctx, factory.NewLogger("test-clash"), "Rule", nil))
	instance, err := NewServer(ctx, factory.(log.ObservableFactory), option.ClashAPIOptions{Secret: "test-secret", ExternalUI: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	server := instance.(*Server)
	t.Cleanup(func() { _ = server.Close(); _ = factory.Close() })
