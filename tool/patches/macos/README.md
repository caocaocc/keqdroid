...
The 0.21.1 audit follows upstream application commit
`be01e02683ce9c3c61a86f6162c7878d82bbeab5`. The earlier core upgrade commit
`059ed4f28ff7c15ef00f4903abb7379a8dfa72ac` ships Mihomo `v1.19.31` and
keqrnel binaries with Xray 26.9.9, sing-box `v1.14.1` and sing-tun `v0.9.3`.
The manifest records their exact module commits and archive hashes. The
published keqrnel source is still `38155c34606f77299a62902da11372ff1c1921d7`;
`keqrnel-current-dependencies.patch` explicitly updates its module lock and the
two sing-box 1.14 interfaces (certificate registry and asynchronous DNS method).
This is a source build of those recorded dependencies, not a claim that the old
keqrnel source commit already contained the newer binary's changes.

Upstream Xray now declares Go 1.27, whose Darwin minimum is macOS 13. The
Darwin build applies only a `go.mod` declaration adjustment to Go 1.26 and
continues using the pinned Go 1.26.8 toolchain. It does not downgrade Xray,
REALITY, TLS or their dependencies. Full builds and loopback tests exercise
TLS 1.3, X25519MLKEM768 and authenticated REALITY payloads with Chrome and
Firefox fingerprints, plus rejection of unsupported fingerprints, wrong short
IDs and an incompatible minimum client version. This compatibility evidence
and Mach-O checks do not replace actual macOS 12 device acceptance.

The patch rebase preserves upstream's IPv4/IPv6 UDP bind selection, sing-tun's
new poller cleanup and sing-box's connection tracking/reset. Only the missing
Darwin DNS connection-context ownership remains from the old DoH patch.
Mihomo uses its own `metacubex/sing-tun` and Mips implementation; the SagerNet
route patch belongs only to keqrnel. Mihomo also follows upstream's new
`no_easytier` build tag.

The subsequent upstream Firefox 148 patch changes Mihomo's default Firefox
ClientHello to include X25519MLKEM768 before its standalone X25519 share,
as required by the bundled REALITY server. Darwin applies that same shared
patch unchanged. Explicit legacy fingerprints remain available; dependency
versions and keqrnel inputs are unchanged. The existing Mihomo test overlay
checks the emitted shares, REALITY authentication key and verified local TLS
handshakes, without introducing another build task or changing public DNS.

| Patch | Original behavior and reason to retain | Test basis |
| --- | --- | --- |
| `keqrnel-current-dependencies.patch` | The published source requirements predate the upstream binary upgrade; freeze the exact new dependency graph and adapt the two changed sing-box interfaces. | Complete core build, local DNS and real generated Proxy/TUN configuration parsing. |
| `xray-go126-compat.patch` | Xray declares Go 1.27; the unchanged runtime source compiles with the last Go series supporting macOS 12. | Full build, authenticated local REALITY/MLKEM and TLS/config regressions. |
...
| `singbox-doh-connection-lifetime.patch` | `https_transport.go` passes the initiating request context into a reusable HTTP/2 connection; `Close` now tracks active connections but cannot cancel a pending dial or detach a completed query from its shared Xray context. | keqrnel's real embedded-Xray/loopback HTTP/2 integration tests; separate sing-box lifetime race tests. |
...
| `../mihomo-firefox-148-hello.patch` | The pinned uTLS fork maps Firefox to an older profile without the hybrid key share required by current REALITY. Follow upstream's Firefox 148 profile and common wrapper in TLS, REALITY and JLS. | ClientHello share ordering/authentication-key tests and local TLS certificate/HTTP regressions in the existing Mihomo checkpoint. |
| `../mihomo-reality-client-version.patch` | `component/tls/reality.go` still advertises 1.8.2; the common repository patch keeps it aligned with bundled Xray's 26.9.9 revision. | Source/application and Mihomo compilation checks; server `minClient` interoperability requires node testing. |

`build_cores.py::test_sources` runs the shared overlays and core-specific Darwin
regressions before publishing each component. The normal keqrnel component build runs the sing-box lifetime unit tests,
keqrnel HTTP/2 integration tests and Xray REALITY compatibility overlay as well
as the bootstrap, privileged API and route ownership regressions.
...
connections. Upstream reset closes the old generation and creates a fresh one;
this patch does not replace that behavior. HTTP/1.1 keeps upstream
connection tracking and protocol fallback. On Go 1.26, its tracked wrapper means
`http.Response.TLS` is empty; the regression checks the actual trusted handshake,
rejects an untrusted certificate, and verifies the complete HTTP/1 response.
...
outside authentication (UI, debugging and DoH). The pinned sing-box 1.14.1
...
`singtun-route-ownership.patch` applies to keqrnel
`github.com/sagernet/sing-tun` at the upstream-updated `v0.9.3`. The replacement also applies to
