...
Compare the downloaded PKG SHA-256 with its exact entry in SHA256SUMS before opening it.

1. Quit any existing KEQDIS instance from its menu.
2. Open keqdroid-<version>-macos-<arch>.pkg and follow macOS Installer. Administrator permission
...
Older versions that only recognize DMG updates need one manual PKG upgrade.
Choose arm64 for Apple Silicon or x64 for Intel; the installer checks both
architecture and the macOS 12 minimum before running installation scripts.

To uninstall, quit KEQDIS and run keqdroid-<version>-macos-<arch>-uninstall.pkg. This first restores
