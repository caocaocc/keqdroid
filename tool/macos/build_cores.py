...
OVERLAYS = {
    ...
    "xray": (("xray", "features/dns/localdns"),
             ("xray_reality", "transport/internet/reality")),
...
def test_sources(cores, go, env, sources):
    ...
    if "keqrnel" in cores:
        ...
        run([go, "test", "./features/dns/localdns", "./transport/internet", "./transport/internet/reality",
             "-run", "^TestKeqdis"], sources["xray"], env,
            "test-xray-bootstrap")
        run([go, "test", "-tags", "with_gvisor", "./core/localdns", "./internal/keqdisdns"],
            sources["keqrnel"], env, "test-keqrnel-localdns")
        run([go, "test", "-tags", "with_gvisor", "./experimental/clashapi", "./dns/transport",
             "-run", "^(TestKeqdis|TestHTTPSLifetime)"],
            sources["singbox"], env, "test-singbox-privileged-api")
    if "mihomo" in cores:
        run([go, "test", "-tags", "with_gvisor,no_tailscale,no_zerotier,no_easytier", "./dns", "-run", "^TestKeqdis"],
            sources["mihomo"], env, "test-mihomo-bootstrap")
        run([go, "test", "-tags", "with_gvisor,no_tailscale,no_zerotier,no_easytier", "./hub/route", "-run", "^TestKeqdis"],
