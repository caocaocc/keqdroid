#!/usr/bin/env python3
"""Inspect downloaded PKGs or a legacy DMG without installing or running its code.

By default this verifies the download hash and target commit/architecture (and
disk image integrity for a legacy DMG). --inspect-payload expands both PKGs
into a temporary directory, checks signatures and Mach-O metadata, and
...
def checksum_for_file(text, filename):
    ...
    for line in text.splitlines():
        ...
        # when the requested package is missing from a shared release manifest.
    ...
    if filename not in records:
        raise ValueError("SHA-256 record does not identify the downloaded file")
    return records[filename]


def check_file(path, checksum):
    path = Path(path)
    if not path.is_file() or path.is_symlink():
        raise ValueError("Expected a downloaded regular file")
    actual = file_hash(path)
    if actual != checksum_for_file(Path(checksum).read_text(), path.name):
        raise ValueError("Downloaded file SHA-256 mismatch")
    return actual


def check_download(package, checksum, report_path, arch, commit):
    package, checksum = Path(package), Path(checksum)
    ...
    if package.suffix not in (".pkg", ".dmg"):
        raise ValueError("Expected a downloaded .pkg or legacy .dmg file")
    actual = check_file(package, checksum)
    if package.suffix == ".pkg":
        check_file(report_path, checksum)
    ...
    if package.name != f"keqdroid-{version}-macos-{arch}{package.suffix}":
        raise ValueError("Package filename does not match the verified version and architecture")
    ...
    result = {"sourceCommit": commit, "architecture": arch, "version": version,
              "packageSHA256": actual, "packageFormat": package.suffix[1:],
              "downloadVerified": True, "payloadVerified": False,
              "packageStructureVerified": False}
    if package.suffix == ".dmg":
        result.update(dmgSHA256=actual, diskImageVerified=False)
    return report, result


def check_uninstaller(path, checksum, report):
    expected = f"keqdroid-{report['version']}-macos-{report['architecture']}-uninstall.pkg"
    if Path(path).name != expected:
        raise ValueError("Uninstaller filename does not match the verified version and architecture")
    return check_file(path, checksum)
...
def inspect_stage(stage, report, arch):
    ...
    if info.get("CFBundleShortVersionString") != report["version"]:
        raise ValueError("Application version differs from the release verification report")
    return {"executablesVerified": len(binaries), "clientRequirement": report["clientRequirement"]}


def inspect_packages(package, uninstall_package, report, arch, commit, repository, work):
    ...
    command("/usr/sbin/pkgutil", "--expand-full", uninstall_package, uninstall)
    ...
    return {**result, "payloadVerified": True, "packageStructureVerified": True,
            "installerScriptsVerified": True, "installerSignature": "unsigned",
            "installerExecuted": False, "payloadExecutablesRun": False}


def inspect_payload(package, report, arch, commit, repository, uninstall_package=None):
    with tempfile.TemporaryDirectory(prefix="keqdis-download-verification-") as temporary:
        work = Path(temporary)
        if Path(package).suffix == ".pkg":
            if uninstall_package is None:
                raise ValueError("Direct PKG payload inspection also requires --uninstall-pkg")
            return inspect_packages(Path(package).resolve(), Path(uninstall_package).resolve(),
                                    report, arch, commit, repository, work)
        mount = work / "media"
        mount.mkdir()
        mounted = False
        try:
            command("/usr/bin/hdiutil", "attach", "-readonly", "-nobrowse", "-noautoopen",
                    "-mountpoint", mount, "-plist", Path(package).resolve())
            mounted = True
            return inspect_packages(mount / "Install KEQDIS.pkg", mount / "Uninstall KEQDIS.pkg",
                                    report, arch, commit, repository, work)
        finally:
            if mounted:
...
def main():
    ...
    package.add_argument("--pkg", type=Path, help="Direct installation package")
    package.add_argument("--dmg", type=Path, help="Legacy disk image for historical audits")
    parser.add_argument("--uninstall-pkg", type=Path, help="Matching uninstaller; required for direct PKG payload inspection")
    ...
    try:
        if args.pkg and args.pkg.suffix != ".pkg" or args.dmg and args.dmg.suffix != ".dmg":
            raise ValueError("The download extension must match --pkg or --dmg")
        if args.dmg and args.uninstall_pkg:
            raise ValueError("A legacy DMG already contains its uninstaller")
        package = args.pkg or args.dmg
        report, result = check_download(package, args.sha256, args.report, args.arch, args.commit)
        if args.uninstall_pkg:
            result["uninstallerSHA256"] = check_uninstaller(args.uninstall_pkg, args.sha256, report)
        if sys.platform != "darwin":
            raise ValueError("Package inspection requires macOS")
        if args.dmg:
            command("/usr/bin/hdiutil", "verify", args.dmg.resolve())
            result["diskImageVerified"] = True
        if args.inspect_payload:
            result.update(inspect_payload(package, report, args.arch, args.commit,
                                          args.repository.resolve(), args.uninstall_pkg))
