...
class CoreCheckpointTests(unittest.TestCase):
    ...
    def test_mihomo_firefox_patch_and_tls_regression_belong_to_its_checkpoint(self):
        for name in ("mihomo-firefox-148-hello.patch",
                     "macos/mihomo/firefox_tls_darwin_test.go"):
            with self.subTest(input=name):
                before = self.fingerprints()
                self.change_patch(name)
                self.assert_changed(before, ("mihomo",))
        sources = {"mihomo": self.directory / "mihomo"}
        with patch.object(builder, "run") as commands:
            builder.test_sources(("mihomo",), Path("go"), {}, sources)
        calls = [call.args[0] for call in commands.call_args_list
                 if call.args[1] == sources["mihomo"]]
        self.assertTrue(any("./dns" in command and "^TestKeqdis" in command
    ...
    def test_xray_physical_socket_regressions_run_with_bootstrap_checks(self):
        ...
        self.assertIn("./transport/internet/reality", xray_calls[0])
        self.assertIn("^TestKeqdis", xray_calls[0])

    def test_reality_compatibility_overlay_is_part_of_keqrnel_checkpoint(self):
        before = self.fingerprints()
        self.change_patch("macos/xray_reality/keqdis_compat_darwin_test.go")
        self.assert_changed(before, ("keqrnel",))
        sources = {name: self.directory / name for name in builder.DEPENDENCIES["keqrnel"]}
        with patch.object(builder, "run") as commands:
            builder.test_sources(("keqrnel",), Path("go"), {}, sources)
        singbox_calls = [call.args[0] for call in commands.call_args_list if call.args[1] == sources["singbox"]]
        self.assertIn("./dns/transport", singbox_calls[0])
        self.assertIn("^(TestKeqdis|TestHTTPSLifetime)", singbox_calls[0])
