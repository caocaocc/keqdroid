...
class DownloadChecks(unittest.TestCase):
    ...
    def pkg_fixture(self):
        package = self.dmg.with_suffix('.pkg')
        package.write_bytes(b'package fixture')
        uninstall = package.with_name(package.stem + '-uninstall.pkg')
        uninstall.write_bytes(b'uninstaller fixture')
        self.sha = self.root / 'SHA256SUMS'
        self.sha.write_text(''.join(f'{verify.file_hash(path)}  {path.name}\n'
                                   for path in (package, uninstall, self.report_path)))
        return package, uninstall

    def test_direct_pkg_hash_and_report_do_not_claim_signature_or_payload_verification(self):
        package, uninstall = self.pkg_fixture()
        report, result = verify.check_download(package, self.sha, self.report_path, 'arm64', self.commit)
        self.assertEqual(result['packageFormat'], 'pkg')
        self.assertEqual(result['packageSHA256'], verify.file_hash(package))
        self.assertNotIn('diskImageVerified', result)
        self.assertFalse(result['packageStructureVerified'])
        self.assertFalse(result['payloadVerified'])
        self.assertEqual(verify.check_uninstaller(uninstall, self.sha, report), verify.file_hash(uninstall))
        self.report_path.write_text(self.report_path.read_text() + '\n')
        with self.assertRaisesRegex(ValueError, 'SHA-256 mismatch'):
            verify.check_download(package, self.sha, self.report_path, 'arm64', self.commit)

    def test_direct_pkg_rejects_uninstaller_as_install_and_wrong_uninstaller_hash_or_arch(self):
        package, uninstall = self.pkg_fixture()
        with self.assertRaisesRegex(ValueError, 'Package filename'):
            verify.check_download(uninstall, self.sha, self.report_path, 'arm64', self.commit)
        with self.assertRaisesRegex(ValueError, 'Uninstaller filename'):
            verify.check_uninstaller(uninstall.with_name(uninstall.name.replace('arm64', 'x64')), self.sha, self.report)
        uninstall.write_bytes(b'changed')
        with self.assertRaisesRegex(ValueError, 'SHA-256 mismatch'):
            verify.check_uninstaller(uninstall, self.sha, self.report)

    def test_direct_pkg_payload_inspection_does_not_mount_or_execute_code(self):
        package, uninstall = self.pkg_fixture()
        with mock.patch.object(verify, 'inspect_packages', return_value={'payloadVerified': True}) as inspect, \
                mock.patch.object(verify, 'command') as command:
            result = verify.inspect_payload(package, self.report, 'arm64', self.commit, self.root, uninstall)
        self.assertTrue(result['payloadVerified'])
        self.assertEqual(inspect.call_args.args[:2], (package.resolve(), uninstall.resolve()))
        command.assert_not_called()
        with self.assertRaisesRegex(ValueError, 'requires --uninstall-pkg'):
            verify.inspect_payload(package, self.report, 'arm64', self.commit, self.root)
    ...
    def test_codesign_fingerprint_is_a_literal_requirement_instead_of_a_filename(self):
        ...
        (app / "Contents/Info.plist").write_bytes(plistlib.dumps({
            ...
            "CFBundleShortVersionString": self.report['version']}))
        ...
        with mock.patch.object(verify, "command"), self.assertRaisesRegex(ValueError, 'Application version'):
            verify.inspect_stage(stage, dict(report, version='0.99.0'), "arm64")
