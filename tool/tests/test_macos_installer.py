...
if name == 'id' or name == 'stat':
    ...
    if args[0] == 'print':
        ...
        started = (root / 'bootstrapped').exists()
        if started and os.environ.get('FAKE_STATUS_READ_FAILURE'):
            print('state = running')
            sys.exit(5)
        if started and os.environ.get('FAKE_STATUS_NOT_RUNNING'):
            print('state = waiting')
        elif os.environ.get('FAKE_LONG_STATUS'):
            # Like launchctl, fail with SIGPIPE if grep -q closes its input
            # after the early state line while the producer is still writing.
            signal.signal(signal.SIGPIPE, signal.SIG_DFL)
            print('state = running', flush=True)
            time.sleep(0.02)
            print('additional launchctl detail\n' * 8192)
        else:
            print('state = running')
    elif args[0] == 'bootout':
        if not loaded.exists(): sys.exit(1)
...
class InstallerTests(unittest.TestCase):
    ...
    def test_running_service_with_long_status_output_is_not_rolled_back(self):
        result = self.execute('postinstall', FAKE_LONG_STATUS='1')
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual((self.runtime / 'identity').read_text(), 'new')
    ...
    def test_status_read_failure_rolls_back_even_if_output_says_running(self):
        result = self.execute('postinstall', FAKE_STATUS_READ_FAILURE='1')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('Postinstall readiness: launchctl print failed (exit 5)', result.stderr)
        self.assert_old_intact()
        self.assertFalse(self.backup.exists())

    def test_nonrunning_service_status_rolls_back_with_readiness_error(self):
        result = self.execute('postinstall', FAKE_STATUS_NOT_RUNNING='1')
        self.assertNotEqual(result.returncode, 0)
        self.assertIn('Postinstall readiness: network service is not running', result.stderr)
        self.assert_old_intact()
        self.assertFalse(self.backup.exists())
