...
REQUEST_NAMES += tuple(
    f"{core}-{mode}-dns-preferences"
    for core in ("xray", "mihomo") for mode in ("proxy", "tun")
)
REQUEST_NAMES += tuple(
...
class MacOSSessionContractTests(unittest.TestCase):
    ...
    def test_dns_preferences_do_not_allow_files_or_executables(self):
        for core in ("xray", "mihomo"):
            original = json.loads((self.directory / f"{core}-tun-dns-preferences.json").read_text())
            component = "keqrnel" if core == "xray" else "mihomo"
            for option in ("path", "certificate_path", "command"):
                with self.subTest(core=core, option=option), tempfile.TemporaryDirectory() as temporary:
                    request = copy.deepcopy(original)
                    config = json.loads(request["configurations"][component])
                    # The legitimate DoH URL path must not grant any general
                    # filesystem or executable exception to DNS containers.
                    config["dns"][option] = "/private/var/root/contract-must-not-be-opened"
                    request["configurations"][component] = json.dumps(config)
                    path = Path(temporary) / "request.json"
                    path.write_text(json.dumps(request))
                    result = self.validate(path)
                    self.assertNotEqual(result.returncode, 0, "Unsafe DNS preference accepted")
                    self.assertIn(option, result.stderr)
