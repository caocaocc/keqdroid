...
class PackagingTests(unittest.TestCase):
    ...
    def test_checksums_cover_this_architecture_without_waiting_for_the_other(self):
        with tempfile.TemporaryDirectory() as temporary:
            work = Path(temporary)
            (work / 'keqdroid-0.20.0-macos-x64.pkg').write_bytes(b'unrelated stale output')
            files = [work / f'keqdroid-0.20.0-macos-arm64{suffix}'
                     for suffix in ('.pkg', '-uninstall.pkg', '-verification.json', '-README.txt')]
            for path in files:
                path.write_bytes(path.name.encode())
            package.write_checksums(files)
            expected = ''.join(f'{package.sha(path)}  {path.name}\n' for path in sorted(files))
            self.assertEqual((work / 'SHA256SUMS').read_text(), expected)
            self.assertEqual(list(work.glob('*.sha256')), [])
            with self.assertRaises(ValueError):
                package.write_checksums([files[0], files[0]])
