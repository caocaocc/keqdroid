#!/usr/bin/env python3
"""Build architecture-specific PKGs with ad-hoc signed payloads. Never installs them."""
import argparse
import hashlib
import json
import os
...
def write_checksums(files):
    ...
    files = list(files)
    if not files or len({path.parent for path in files}) != 1 or len({path.name for path in files}) != len(files):
        raise ValueError('Checksums require unique release files in one output directory')
    records = ''.join(f'{sha(path)}  {path.name}\n' for path in sorted(files))
    (files[0].parent / 'SHA256SUMS').write_text(records)
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='keqdis-package-', dir=args.output) as temporary:
        ...
        for uninstall in (False, True):
            ...
            output = args.output / f'keqdroid-{version}-macos-{args.arch}{suffix}.pkg'
            packaged = work / output.name
            run('/usr/bin/productbuild', '--distribution', distribution, '--package-path', work, packaged)
            packaged.replace(output)
            release_files.append(output)
        instructions = args.output / f'keqdroid-{version}-macos-{args.arch}-README.txt'
        shutil.copy2(ROOT / 'tool/macos/installer/README.txt', instructions)
        ...
        print(f'Created {release_files[0]}. Runtime installation and macOS 12 acceptance are still required.')
