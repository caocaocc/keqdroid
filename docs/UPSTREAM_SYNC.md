...
## 2026-09-24：折叠手动取消与连续切换修复

上游继续固定为 `ec4527ad54a0f03d5cd9ef57f107a2b59a46498c`（0.22.0+53）。本轮旧 dev `c3d09e7519e5a87f08348fa43d54357e427ea692` 已备份到本地和 origin 的 `codex/dev-backup-c3d09e7`。修复与对应测试折叠入分类 2，发行说明和本记录归入分类 4；保持上游之后恰好四个分类提交，无合并提交。

| 分类 | 原提交 | 折叠后的提交 |
| --- | --- | --- |
| 1 通用功能 | `4342327ed28126ac45b3a08128ae74d2bc87e454` | 不变 |
| 2 macOS 支持 | `9ead54aab061247d3e0bc7df25244540854b16b4` | `d5ea526ce84c83de3a295e0c7a29c0c07a734a93` |
| 3 发行配置 | `7268b816b99bcf57d3cd66ed203ffe244448a9d8` | `dda4ee732799af8113c4e3ec16b9b82136ef6109` |
| 4 CI 与发行记录 | `c3d09e7519e5a87f08348fa43d54357e427ea692` | 本记录所在最终提交，完整 SHA 见构建报告 |

- 手动断开和取消立即使切节点、切模式的旧异步回调失效；连续操作等待当前清理完成，仅最后有效的选择可以启动。复用现有连接代次、状态和清理流程，不新增存储或连接状态。
- 服务器页、macOS 菜单与快捷键、Windows 托盘共用手动选择入口。窗口恢复和管理员确认等异步等待同样校验操作是否仍有效；取消管理员确认不影响原会话。
- 切换过程中停止旧核心仍可取消后续启动；普通断开保持防重复操作。选择当前节点不会取消其现有 macOS 恢复意图。
- 确定性测试在旧代码上复现了保存/选择延迟回调及切换间隙、停止阶段的误启动，修复后通过。现场曾在手动断开后观察到新 PID，但是否有用户同时重连尚未确认，不能将其直接归因于这些源码竞态。
- 核心、helper、原生服务、依赖和工具链均未改变；安装包按最终提交重新生成。已安装的 c3d09e7 不含本轮修复，不能替代新包验收。

本轮本地静态分析无问题，完整 Flutter 回归 2131 项通过、3 项按平台跳过；34 项定向测试中新增 17 项。相同 runner 输入下，四个核心和两个 helper 指纹与 c3d09e7 成功 CI 一致，仅五个平台应用的五个 Dart 文件输入有变化。新目标提交的 CI 与安装验收另行记录。

旧提交 c3d09e7 的全平台 CI `35878204559` 为 20/20 成功，四个核心全部复用。其 arm64 包已完成本机安装：真实安装日志、receipt、CDHash、19 个 Mach-O 与核心清单匹配，无回滚；旧 93 个设置字段、3 个订阅及 108 个节点完整保留。Xray Proxy 的本地 HTTP/SOCKS 端口实际 HTTPS 流量与系统代理已检查，后续 TUN、LAN 和异常恢复尚待本轮目标包验收。Intel、macOS 12 及其他缺少真机环境的场景仍未覆盖。

## 2026-09-23：同步 0.22.0

固定上游 `ec4527ad54a0f03d5cd9ef57f107a2b59a46498c`（`0.22.0+53`），相对旧基线 `be01e02683ce9c3c61a86f6162c7878d82bbeab5` 新增 29 个提交。旧 dev `8f8e2210a25d07797a0e56f717d064830811c1ae` 已保存到本地和 origin 的 `codex/dev-backup-8f8e221`。从明确旧基线重放四类提交，集成修正折叠回所属分类，不创建合并提交。

| 分类 | 原提交 | 重放后的提交 |
| --- | --- | --- |
| 1 通用功能 | `6f32b064f3ffeab524c975fabe7972eab9f24dda` | `4342327ed28126ac45b3a08128ae74d2bc87e454` |
| 2 macOS 支持 | `8bf5104eeb21581ef42b8552a31ce69984902674` | `9ead54aab061247d3e0bc7df25244540854b16b4` |
| 3 发行配置 | `3677b9535805eba638e51ec2292ec189e013403c` | `7268b816b99bcf57d3cd66ed203ffe244448a9d8` |
| 4 CI 与发行记录 | `8f8e2210a25d07797a0e56f717d064830811c1ae` | 本记录所在最终提交，完整 SHA 见构建报告 |

- 保留上游订阅自动选择、五秒桌面流量静默判断、失败信号和当前节点/邻居的实时测量；macOS 接入会话独立累计流量与逐行失败计数。手动选择交回控制，旧测量不能修改已取消或切换后的会话。
- 保留上游 DNS hosts、按域名指定解析器和并发拨号；显式 DNS policy 优先于默认本地域/CN 分工，节点 bootstrap 保持既有防环优先级。升级保留已有设置，新安装仍使用中国默认值。
- 保留 Mihomo 初启短暂拒绝后的重试，并复用桌面 GET 的单一请求截止时间、阶段诊断和连接清理；启动预算与请求预算分开记录。Android 整批核心启动失败仍回退拆分，实时单节点结果保留。
- 保留上游 Windows 首次连接不关闭从未启用的系统代理，以及 Windows/Linux 完整行日志和就绪信号处理。上游精简后的设置文案、Auto 按钮和连接错误布局保持。
- 上一轮修复包 `8f8e221` 已通过安装日志、receipt、应用/核心/helper 摘要确认在本机正常安装；安装脚本 SIGPIPE 修复继续保留。此结果不替代新 `0.22.0` 的安装及网络验收。
- Flutter、Go、Xcode、CocoaPods 和依赖锁保持不变。新应用与有实际改动的 helper 按输入重新构建，未变的核心继续验证复用；正式发布仍需目标提交的 CI 和本机验收通过。

本地静态分析无问题。完整 Flutter 回归通过 2113 项、按平台跳过 3 项，另发现一项旧设置测试误选新增输入框内部的滚动区域；修正测试定位后，该文件 5 项全部通过，保留上游页面布局。最终提交的 CI 会重新执行全量测试。Python 检查 191 项、2 项跳过，原生网络检查 819 项通过；实际 Android Kotlin 源码的 10 项回环测试通过。

DNS 跨层检查覆盖 32 份最终会话请求与 8 份不安全变体，保证合法 DNS 数据中的 `error`、`path` 等名称不会被误认为文件选项，同时保持文件、可执行程序及错误嵌套结构的拒绝规则。自动选择回归覆盖手动接管、旧会话结果、独立流量计数及切换前保存有效测速结果；桌面探测覆盖 Mihomo 初启重试仍受同一截止时间限制。输入指纹确认四个核心不变，两个 helper 和五个平台应用有变化；重写历史不改变组件复用条件。
