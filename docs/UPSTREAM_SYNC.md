...
## 2026-09-22：修复 PKG 服务状态检查误触发回滚

旧 dev `8d8b67194f053fd928aeb65cefc5123dcf927ad2` 已备份到本地和 origin 的 `codex/dev-backup-8d8b671`。上游基线仍为 `be01e02683ce9c3c61a86f6162c7878d82bbeab5`；安装脚本和回归测试折叠进分类 3，记录归入分类 4，保持四个自有提交且不创建合并提交。分类 1、2 不变；分类 3 从 `892d198ba7e30008d9a3d6b9195e7a1c3acc75db` 改为 `3677b9535805eba638e51ec2292ec189e013403c`，分类 4 为本记录所在最终提交。

用户两次安装均在 postinstall 的服务状态检查处失败。`launchctl print | grep -q` 在匹配后提前关闭管道，使 `launchctl` 返回 SIGPIPE（141），被 `pipefail` 误判为失败并回滚。真实只读检查稳定复现；旧应用、核心、恢复记录及偏好完整保留，安装前后代理、DNS 和路由一致。

修复先完整读取 `launchctl` 状态，再判断服务是否运行。读取错误或未运行仍报出明确的 readiness 阶段错误并回滚；保留原两秒等待，不延长超时或跳过完整性检查。新增长输出、读取失败但输出含 running、未运行三项测试，旧脚本三项均失败，修复后全部 16 项安装回归通过；修复后的真实只读检查也通过。

11 个组件在相同工具链和 runner 镜像输入下指纹均未变化，复用已有可信编译产物，仅安装包需重新生成。原提交 CI 20/20 成功不代表本机安装通过；新包须重新校验并完成安装验收后才可发布。

## 2026-09-22：同步批量测速与 Firefox 握手更新

固定上游 `be01e02683ce9c3c61a86f6162c7878d82bbeab5`，版本仍为 `0.21.1+52`，相对旧基线 `36d8b1655b550feebe65044b8ad54df93b6be9bc` 新增 9 个提交。旧 dev `ac634f61f3694876d3141aeb3682125de919430b` 已保存到本地和 origin 的 `codex/dev-backup-ac634f6`。从明确旧基线重放四类提交，没有合并提交；后续修复折叠回对应分类。

| 分类 | 原提交 | 重放后的提交 |
| --- | --- | --- |
| 1 通用功能 | `d2b96070f33684f5204bd097ab3285047297818c` | `6f32b064f3ffeab524c975fabe7972eab9f24dda` |
| 2 macOS 支持 | `8f83080115efe4122a7d50e7b4e3384d350242b0` | `8bf5104eeb21581ef42b8552a31ce69984902674` |
| 3 发行配置 | `db266fce650c0ff86849d10bb3c3ba8b98c90bd5` | `892d198ba7e30008d9a3d6b9195e7a1c3acc75db` |
| 4 CI 与发行记录 | `ac634f61f3694876d3141aeb3682125de919430b` | 本记录所在最终提交，完整 SHA 见构建报告 |

- 保留 Android 单核心批量测速、按核心分组和拆分回退，以及桌面 16 路逐节点并发。macOS 沿用新增接口默认的不支持返回值，继续使用原有临时核心、GET 诊断、直连 DNS 和物理 DNS 快照。
- Linux 保留上游 root 直接启动和缺少 polkit 的结构化提示；非 root 分支实际执行已解析的 `pkexec` 路径，避免 PATH 缺少 `/usr/bin` 时找到程序却仍无法启动。回归覆盖提权参数及 root 分支。
- Android 整批核心未能启动或端口未就绪时，返回已有协议的空列表，让 Dart 沿用已有拆分回退；逐节点网络测量失败继续保留原结果。
- 保留滚动隐藏添加按钮、粘贴提示翻译和 Linux 已精简核心；macOS 状态提示仍在既有连接区域，不新增页面顶部布局。
- macOS mihomo 显式纳入上游公共 Firefox 148 ClientHello 补丁，按当前锁定来源重新检查与 Darwin DNS/API 补丁的组合。核心模块版本和 Go 1.26.8 不变，不将 Linux stripping 或 root GUI 行为迁入 macOS。
- 上一次目标提交的 CI 为 20/20 通过；本机现有 `0.21.1 build 52` 指纹确认属于旧 `ac634f6`。版本号相同不代表新提交已安装，本次构建、下载校验和安装验收分别绑定最终提交。

本地 analyze 无问题，完整 Flutter 2001 项通过、3 项按平台跳过；最终 Python/原生回归 186 项、2 项跳过。实际 Kotlin 源码的 8 项 JVM 回归通过，其中 6 项故障回退在旧上游源码上确实失败；该检查仅替代 Android/JNI 边界，不代替 Android 设备验收。Mihomo 通过 Go 1.26.8 编译、证书验证、MLKEM 与固定 Xray REALITY 真实回环认证/载荷测试。

两架构 keqrnel 和 helper 的 CI 输入指纹均与上次成功构建一致，Mihomo 按补丁及测试变化重新构建。完整提交对应关系记录在折叠后的分类表和交付报告中；最终发行仍需目标提交 CI 与安装验收通过。
