...
基于上游 Lemonochka/keqdroid `b85d0bf47d2d977544c472d444d9c0a149f110f9`（0.20.0+48），保留产品名 **KEQDIS**。本仓库修改整理为通用功能、macOS 支持、发行配置、CI 与发布四类。

## 本次变化

- 保留上游新版服务器编辑器、按核心显示的设置、Windows 管理员登录启动及服务器图标配色。
...
- **macOS**：按处理器下载 `keqdroid-0.20.0-macos-arm64.pkg` 或 `keqdroid-0.20.0-macos-x64.pkg`。安装前断开并退出 KEQDIS，在系统界面完成管理员授权。内部应用和组件使用 ad-hoc 签名，安装包未经 Apple 公证。旧版仅识别 DMG 的更新器需要手动安装本次 PKG，此后使用 PKG 更新。卸载包名称包含 `-uninstall`，默认保留用户配置。
