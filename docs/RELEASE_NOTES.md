...
基于上游 Lemonochka/keqdroid `ec4527ad54a0f03d5cd9ef57f107a2b59a46498c`（0.22.0+53），保留产品名 **KEQDIS**。本仓库修改整理为通用功能、macOS 支持、发行配置、CI 与发布四类。

## 本次变化

- 同步上游订阅自动选节点：结合核心流量与连接失败信号，重新测量当前节点和邻居后选择可用节点；手动选择节点会关闭自动选择。
- 同步 DNS hosts、按域名选择解析器和并发拨号设置；显式解析规则优先于默认中国分流，保留 macOS 物理 DNS 与防环路径。
- 同步连接和测速提速、按完整行读取核心日志及 Windows 系统代理关闭时机修复。macOS 接入同一自动选择机制，窗口隐藏时仍可检查节点，旧会话测量不会切换新会话。
- 修复切节点或切模式过程中取消仍可能被旧操作重新连接的问题；连续切换保留最后一次选择，复用现有连接状态和清理流程。
- 修复 macOS PKG 服务状态读取的 SIGPIPE 误回滚；保留安装失败时恢复旧版本和设置的行为。
...
- **macOS**：按处理器下载 `keqdroid-0.22.0-macos-arm64.pkg` 或 `keqdroid-0.22.0-macos-x64.pkg`。安装前断开并退出 KEQDIS，在系统界面完成管理员授权。内部应用和组件使用 ad-hoc 签名，安装包未经 Apple 公证。旧版仅识别 DMG 的更新器需要手动安装本次 PKG，此后使用 PKG 更新。卸载包名称包含 `-uninstall`，默认保留用户配置。
