...
本机只有 Command Line Tools，完整应用由 GitHub CI 构建。构建、安装和真实网络验收分别记录，具体覆盖范围以对应 Release 的验收报告为准；新系统编译成功不能当作 macOS 12 已验证。分段构建、复用和本机校验方法见 [MACOS_CI.md](MACOS_CI.md)。
...
| `tool/package_macos.py` | 检查、逐层签名、安装/卸载 PKG 和 SHA-256 |
...
恢复提示复用连接按钮下方的固定状态区，不在页面顶部插入横幅。点击状态文字查看详细原因，清理失败时仍提供“重试恢复网络”。已连接时的 DNS 警告通过节点标签旁的图标查看，标签本身仍可跳转到活动节点；完整结果也保留在诊断页。
...
```text
keqdroid-<version>-macos-arm64.pkg
keqdroid-<version>-macos-arm64-uninstall.pkg
keqdroid-<version>-macos-arm64-verification.json
keqdroid-<version>-macos-arm64-README.txt
keqdroid-<version>-macos-x64.pkg
keqdroid-<version>-macos-x64-uninstall.pkg
keqdroid-<version>-macos-x64-verification.json
keqdroid-<version>-macos-x64-README.txt
SHA256SUMS
```

每个架构独立输出四个文件及 `SHA256SUMS`，清单覆盖本次安装包、卸载包、验证报告和说明，不等待另一架构，也不包含历史产物。独立 CI 产物中的同名清单应分别保存；Release 发布流程收齐各平台产物后生成覆盖全部资产的统一清单。
...
下载后先验证，再打开安装包；以下示例使用 `0.22.0` 的 arm64 产物，在文件所在目录执行，并将源码路径和提交替换为实际值：

```sh
python3 /path/to/keqdroid/tool/macos/verify_download.py \
  --pkg keqdroid-0.22.0-macos-arm64.pkg \
  --uninstall-pkg keqdroid-0.22.0-macos-arm64-uninstall.pkg \
  --sha256 SHA256SUMS \
  --report keqdroid-0.22.0-macos-arm64-verification.json \
  --arch arm64 --commit <完整提交SHA> --inspect-payload
```

检查包括下载及报告摘要、实际应用版本、Mach-O 架构/最低系统版本、内部签名和安装脚本。它不执行包内代码或修改网络，也不将内部签名验证称为 PKG 的 Apple 签名。

## 更新、卸载与中断恢复

所有平台的应用更新及 Geo 下载源统一为 [`caocaocc/keqdroid`](https://github.com/caocaocc/keqdroid/releases)，保留原始项目和数据来源说明。macOS 更新器仅选择与目标 Release 版本和当前 ABI 精确匹配的安装 PKG，排除卸载包、其他架构及其他平台资产。下载并验证 `SHA256SUMS` 后，先恢复网络，再打开 Installer，成功后正常退出 GUI；清理或打开失败时保留界面报告错误。不做静默自替换。

旧版客户端仅识别 DMG，无法发现新的 PKG 资产，需要从本仓库 Release 手动下载并安装一次。安装保留原有 Bundle ID、安装位置和用户配置。卸载使用同版本、同架构的 `keqdroid-<version>-macos-<arch>-uninstall.pkg`。
...
`dev` CI 保存开发产物。完成构建检查和本机验收后，将本仓库版本标签指向最终提交；独立发布工作流只收集该提交已成功的可信构建，验证各平台文件及摘要，先上传草稿，再发布稳定 Latest Release，不重新编译。发布同时包含 Android、Windows、Linux 和 macOS 两架构产物。

下表用于逐项记录验证范围；未执行场景明确标为未覆盖。发布稳定版本不代表每个目标系统和网络场景都已通过真机验收，尤其不能据较新 macOS 的构建结果宣称 macOS 12 已验证。
...
| M4 | 双架构 PKG、升级授权/取消、安装失败回滚、卸载；现有 PAC/DNS/搜索域与用户中途修改；完整回归通过 |

网络场景还须覆盖 Wi-Fi/有线切换、睡眠唤醒、断网重连、IPv4/双栈、GUI/核心/helper 崩溃、连续快速连接断开和另一个全局 VPN 的冲突。macOS 12 Intel 与 Apple Silicon 的支持结论需要对应真机证据；macOS 13、15、当前稳定版本补充覆盖。
